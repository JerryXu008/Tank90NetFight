...
+(void)danji{
    ...
    {
        [sharedInstance.viewController dismissViewControllerAnimated:YES completion:^{
        
        DO_DATA_CALLBACK(danjiOnly, nil);
        }];
    }
...
+ (void) assignViewController: (UIViewController *) aViewController
{
	...
	if (sharedInstance.viewController)
    {
         sharedInstance.viewController.navigationItem.rightBarButtonItem = BARBUTTON(@"连接", @selector(connect));
         sharedInstance.viewController.navigationItem.leftBarButtonItem = BARBUTTON(@"单机", @selector(danji));
    }
}
